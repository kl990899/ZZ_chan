class Post < ApplicationRecord
  ...
  def validate_image_content_type
    ...
    return if image.content_type.in?(['image/jpeg', 'image/png', 'image/gif', 'image/webp'])
  ...
  def validate_video_content_type
    ...
    return if video.content_type.in?(['video/mp4', 'video/webm', 'video/ogg', 'video/quicktime'])
  ...
  def validate_video_size
    ...
    return unless video.byte_size > 5.megabytes

    errors.add(:video, '影片大小不能超過 5MB')
  end

  def cannot_have_both_image_and_video
    return unless image.attached? && video.attached?
