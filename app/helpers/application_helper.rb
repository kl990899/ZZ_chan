module ApplicationHelper
  ...
  def clean_remaining_iframe_text(content)
    # 移除任何殘留的 iframe 相關文字
    content.gsub(%r{<iframe[^>]*>.*?</iframe>}mi, '').strip
  end
  ...
  def process_existing_iframes(content)
    ...
    iframe_patterns = [
      # 標準的 iframe 標籤
      %r{<iframe[^>]*src=["']https?://(?:www\.)?youtube\.com/embed/([a-zA-Z0-9_-]+)[^>]*>.*?</iframe>}mi,
      # 不完整的 iframe 標籤（沒有結束標籤）
      %r{<iframe[^>]*src=["']https?://(?:www\.)?youtube\.com/embed/([a-zA-Z0-9_-]+)[^>]*>}mi,
      # 包含額外參數的 iframe
      %r{<iframe[^>]*src=["']https?://(?:www\.)?youtube\.com/embed/([a-zA-Z0-9_-]+)\?[^"']*["'][^>]*>.*?</iframe>}mi
    ]
